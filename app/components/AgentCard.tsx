    ...
    <>
      ...
      >
        ...
        <div className="p-5 pb-4 flex flex-col gap-3">
          ...
          {(agent.purchases > 0 || agent.feedbackTotal > 0) && (
            <div className="flex items-center gap-2 text-[11px]">
              {agent.purchases > 0 && (
                <span className="flex items-center gap-1 py-0.5 px-1.5 rounded-md bg-[#f0f0ee] text-text-2">
              ...
              {agent.feedbackPositive > 0 && (
                <span className="flex items-center gap-1 py-0.5 px-1.5 rounded-md bg-[#e8f5e9] text-[#4caf50]">
              ...
              {agent.feedbackNegative > 0 && (
                <span className="flex items-center gap-1 py-0.5 px-1.5 rounded-md bg-[#fce4ec] text-[#ef5350]">
        ...
        <div className="flex items-center justify-between mt-auto px-5 py-3.5 border-t border-border">
          {agent.price === "Free" ? (
            <span className="text-sm font-semibold text-text-2">Free</span>
          ) : (
            <div className="flex items-baseline gap-1.5">
              <span className="text-sm font-bold text-text">{agent.price}</span>
              <span className="text-[11px] text-text-2">per task</span>
            </div>
          )}
          <span className="text-[11px] text-accent font-medium">Details &rarr;</span>
        </div>
      </div>
